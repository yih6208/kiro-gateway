...
async def messages(
    ...
    try:
        kiro_payload, has_server_side_tools = anthropic_to_kiro(
            request_data,
            conversation_id,
            profile_arn_for_payload
        )
    ...
    try:
        ...
        if request_data.stream:
            ...
            async def stream_wrapper():
                ...
                try:
                    ...
                    if has_server_side_tools:
                        # Use server tool loop for requests with web_search/web_fetch
                        from kiro.server_tools import stream_with_server_tool_loop
                        logger.info("[Streaming] Server-side tools detected, using tool execution loop")
                        generator = stream_with_server_tool_loop(
                            response,
                            auth_manager,
                            http_client,
                            converted_model_id,
                        ...
                        generator = stream_kiro_to_anthropic(
                            response,
                            converted_model_id,
                            model_cache,
                            auth_manager,
            ...
            if has_server_side_tools:
                from kiro.server_tools import collect_with_server_tools
                logger.info("[Non-streaming] Server-side tools detected, using server tool collection")
                anthropic_response = await collect_with_server_tools(
                    response,
                    auth_manager,
                    http_client,
                    converted_model_id,
